...
export async function GET(req: Request) {
 ...
 const saleType = searchParams.get("sale_type");
  ...
  const { data: auctionsData,count, error: countError } = await supabase
    .from("auctions")
    .select(`id, productname, currentbid, productimages, startprice, auctiontype, auctionsubtype, categories:categoryid(handle), scheduledstart,  auctionduration, buy_now_price`,
       { count: "exact"})
