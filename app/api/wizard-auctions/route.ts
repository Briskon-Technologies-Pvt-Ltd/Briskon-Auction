...
let saleType = 1; // default: auction

if (auctionData.buy_now_price && auctionData.startPrice) {
  // Auction + Buy Now
  saleType = 3;
} else if (auctionData.buy_now_price && !auctionData.startPrice) {
  // Buy Now only
  saleType = 2;
} else {
  // Auction only (forward/reverse)
  saleType = 1;
...
if (saleType === 1 || saleType === 3) {
  if (auctionData.bidIncrementType === "fixed") {
    ...
    if (isNaN(incrementValue) || incrementValue < 0) {
      return NextResponse.json(
        { success: false, error: "Minimum increment must be zero or a positive number for fixed type" },
        { status: 400 }
      );
    }
  }
...
const { city, state, country, ...restData } = auctionData as any;

// ✅ Format combined location string
const formattedLocation = [city, state, country].filter(Boolean).join(", ");
    let auctionInsertData: any = { ...restData };

if (saleType === 2) {
  const { auctionType, auctionSubType, ...rest } = auctionInsertData;
  auctionInsertData = rest;
}
    ...
    const newAuction = keysToLowerCase({
      ...
      sale_type: saleType,  // 👈 store as 1,2,3
      buy_now_price: auctionData.buy_now_price || null, 
      percent:
      ...
      createdby: createdBy,
      ...(auctionData.auctionSubType === "sealed" ? { evaluationcriteria: auctionData.evaluationCriteria } : {}),
