...
const lowestBidsMap = bids.reduce((map, bid) => {
  const existing = map.get(bid.user_id);
  // Keep the lowest bid per user
...
const uniqueBids = Array.from(lowestBidsMap.values()).sort(
  (a, b) => a.amount - b.amount // ascending: lowest first
);
