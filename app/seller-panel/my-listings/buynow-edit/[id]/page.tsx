...
import AuctionBuilderReverse from "@/app/createReverse/page";
import BuyNowBuilder from "@/app/sellerPanel/create-buy-now/page";
    ...
    productImages: (auction.productimages ?? []).map((url) => {
      ...
      const type =
        ext === "jpg" || ext === "jpeg"
          ...
          : ext === "gif"
          ? "image/gif"
          : undefined; // fallback if unknown
      return {
        id: url, // Use url as the unique id if it's unique
    ...
    <div className="p-10 bg-gray-100">
      <div className="min-w-44 p-10">
        <div className="max-w-7xl mx-auto rounded-lg bg-white dark:bg-gray-900 shadow-sm border-0">
          <BuyNowBuilder initialData={initialData} isEdit />
        </div>
      </div>
    </div>
  );
