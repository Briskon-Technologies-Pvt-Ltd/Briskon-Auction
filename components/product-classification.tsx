...
import { fetchCategories, fetchSubcategories, fetchAttributes } from "@/Data/product-data-service"
import { createCustomAttribute } from "@/Data/product-attributes"
...
useEffect(() => {
  const loadCategories = async () => {
    ...
    try {
      ...
      console.log("✅ Supabase categories fetched:", cats)
      setCategories(cats)
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const loadSubcategories = async () => {
      const subs = await fetchSubcategories(category.handle || category.name.toLowerCase())
  ...
  useEffect(() => {
    const loadAttributes = async () => {
      ...
      const attrs = await fetchAttributes(targetId)
      const hasCondition = attrs.some((a) => a.id === "condition")
      ...
      const existingCustomAttrs = attributes.filter((a) => a.id.startsWith("custom-"))
      const merged = [
        ...attrs.map((attr) => {
  ...
  useEffect(() => {
    if (subCategoryId && subcategories.length > 0) {
      ...
      if (sub) setSubCategorySearch(sub.name)
    }
      ...
      {attributes.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="{className} text-xl font-bold text-gray-800 dark:text-gray-100">Attributes</h3>
